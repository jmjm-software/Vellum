...
- The launcher widget is a separate, compact surface: its components are the widget part of the design (or a starter view when you design none). dashboard_preview returns widget screenshots (widget-mirror-*) whenever the design has a widget presentation; they are drawn by a layout mirror that shares its tokens with the real launcher widget, so they are faithful but approximate — launcher chrome (cell padding, corner masking, dynamic colors) is not included, and the review records widget_preview_approximate. Inspect them: image/component sizes, truncation (maxItems), empty states. Set a widget image's size (small|medium|large) rather than expecting a fixed look.
